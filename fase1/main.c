...
int main () {
    ...
    while (WindowShouldClose() == false && end == false && timer3 < 120){
        ...
        if (read == true){
            ...
            if (timer2 == 0) strcpy(str_tips, word[1]);
            rewind(words);
            rewind(tips);
            timer1 = 0;
            if (timer2 != 0) change = true;
            idx = 0;
            read = false;
        }
        ...
        if (up_life == true){
            DrawUpLife(str_cura, &up_life, &opacidade1, &scroll1, &cura);
        }
        if (down_life == true){
        ...
        DrawTip(str_tips, &timer2, &scroll3, &opacidade3);
